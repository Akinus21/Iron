...
/// Tracks a single in-flight or completed download.
...
/// Manages WebKit downloads: intercept, route to XDG_DOWNLOAD_DIR,
/// notify on finish, and expose a history list.
...
impl DownloadManager {
    ...
    /// Register `connect_download_started` on the `NetworkSession` derived
    /// from `webview`.  This must be called once per WebView.
    pub fn attach(view: &webkit6::WebView, mgr: Rc<RefCell<DownloadManager>>) {
        ...
        session.connect_download_started(move |_sess, dl| {
            ...
            // If a file already exists, append a counter like (1), (2), etc.
            ...
            // consume the request — returning true from a closure that takes
            // no return value just means we let it continue.  Since `connect_download_started`
            // does not ask for a return value, we simply set the destination above.
        });
    }

    /// Return only completed (successful) downloads, newest first.
...
/// Send a Gio desktop notification when a download finishes.
...
/// Open the folder containing `path` in the user's default file manager via xdg-open.
...
/// Append (1), (2), … to `path` until it no longer exists.
